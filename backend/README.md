...
```dotenv
LLM_RESPONSES_URL=https://api.deepseek.com/responses
LLM_API_KEY=<deepseek-api-key>
LLM_MODEL=deepseek-v4-flash
```

当前默认接 DeepSeek Responses API。简历解析属于结构化抽取任务，默认使用
`deepseek-v4-flash` 控制延迟和成本；如果更看重复杂履历的抽取质量，可以把
`LLM_MODEL` 切换为 `deepseek-v4-pro`，其余后端代码无需改动。
