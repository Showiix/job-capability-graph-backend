...
SQLITE_DB = Path(
    os.getenv("JD_SQLITE_DB", JD_DATA_DIR / "outputs" / "tech_signals.sqlite")
)
OUTPUT_FILE = Path(
    os.getenv(
        "JD_GRAPH_OUTPUT_FILE",
        Path(__file__).parent.parent / "app" / "graph" / "jd_graph_data.json",
    )
)
...
def load_csv_files() -> tuple[
    dict[str, list[dict[str, str]]], dict[str, set[str]], int
]:
    ...
    for csv_file in csv_files:
        ...
        try:
            with open(
                csv_file, encoding="utf-8-sig", newline="", errors="ignore"
            ) as handle:
                reader = csv.DictReader(handle)
                for row in reader:
                    source = normalize_source(
                        row.get("source") or csv_file.parent.name, csv_file.parent.name
                    )
                    row["_source"] = source
    ...
    message = (
        f"{len(csv_files)} 个 CSV，{len(jobs_by_category)} 个岗位类别，"
        f"{total_jobs} 条 JD"
    )
    print("已读取 " + message)
...
def load_sqlite_trends() -> dict[str, dict[str, float]]:
    ...
    try:
        with sqlite3.connect(str(SQLITE_DB)) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT keyword, last_count, last_burst_score FROM keyword_stats"
            )
...
def role_domain(category: str) -> str:
    ...
    if any(
        token in base
        for token in ("芯片", "嵌入式", "单片机", "电子", "FPGA", "物联网")
    ):
        return "智能硬件"
...
def build_graph(
    ...
    for category in categories:
        ...
        for row in rows:
            ...
            text = " ".join(
                (row.get(column) or "")
                for column in ("job_name", "skill_requirements", "tech_tags")
            )
            ...
            sample_key = sample["url"] or "|".join(
                (sample["jobName"], sample["companyName"], sample["city"])
            )
            if (
                sample["jobName"]
                and sample_key not in seen_sample_keys
                and len(samples) < 5
            ):
    ...
    for index, category in enumerate(categories):
        ...
        category_colors[category] = (
            "#ee1212" if is_emerging else ROLE_COLORS[index % len(ROLE_COLORS)]
        )
        ...
        bonus_skills = [
            skill
            for skill, _ in frequency.most_common(10)
        ...
        stars.append(
            {
                ...
                "size": round(
                    0.86 + min(len(jobs_by_category[category]) / 900, 0.62), 3
                ),
                ...
                "sourceCounts": dict(
                    Counter(
                        ...
                        for row in jobs_by_category[category]
                    )
                ),
        ...
        for skill_index, (skill, frequency_count) in enumerate(
            ...
            related_categories = sum(
                skill in freq for freq in category_skill_frequency.values()
            )
            orbit_radius = (
                2.35 + (skill_index if is_required else skill_index + 3) * 0.46
            )
            type_colors = {
                "core": "#ee1212",
                "foundation": "#dad0c8",
            ...
            planets.append(
                {
                    ...
                    "confidence": round(
                        min(98, 52 + (frequency_count / max_frequency) * 46), 1
                    ),
