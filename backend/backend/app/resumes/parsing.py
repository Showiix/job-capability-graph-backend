...
def _extract_docx_text(path: Path) -> str:
    ...
    textbox_text = _extract_docx_xml_paragraphs(
        document_xml,
        ".//w:txbxContent//w:p",
    )
    if textbox_text.strip():
        return textbox_text
    return _extract_docx_xml_paragraphs(document_xml, ".//w:body/w:p")


def _extract_docx_xml_paragraphs(xml_bytes: bytes, selector: str) -> str:
    root = ET.fromstring(xml_bytes)
    values: list[str] = []
    previous: str | None = None
    for paragraph in root.findall(selector, DOCX_NAMESPACES):
        text = "".join(
            (node.text or "") for node in paragraph.findall(".//w:t", DOCX_NAMESPACES)
        ).strip()
        if not text:
            continue
        if text == previous:
