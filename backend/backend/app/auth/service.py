...
GUEST_USERNAME = "guest_applicant"
GUEST_DISPLAY_NAME = "访客候选人"
...
async def guest_login(
    ...
    user = await db.scalar(
        select(User).where(User.username_normalized == GUEST_USERNAME)
    )
    if user is None:
        user = User(
            username=GUEST_USERNAME,
            username_normalized=GUEST_USERNAME,
            # The guest account has no usable password; it is session-only.
    ...
    if not user.is_active:
        raise APIError(403, "GUEST_ACCESS_DISABLED", "访客会话已停用")
    if user.role != "applicant":
        raise APIError(409, "GUEST_ACCOUNT_CONFLICT", "访客账号配置冲突")
