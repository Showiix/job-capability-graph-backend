...
async def test_guest_login_sets_applicant_session_and_csrf(client) -> None:
    response = await client.post("/api/v1/auth/guest")

    assert response.status_code == 200
    assert response.json()["data"]["role"] == "applicant"
    assert response.json()["data"]["display_name"] == "访客候选人"
    assert response.cookies["session"]
    assert response.cookies["csrf"] == response.json()["data"]["csrf_token"]
    assert "HttpOnly" in response.headers["set-cookie"]
    ...
    assert me.json()["data"]["username"] == "guest_applicant"


async def test_guest_login_is_disabled_outside_local_environments(
    client,
    monkeypatch,
) -> None:
    ...
    assert response.json()["error"]["code"] == "GUEST_ACCESS_DISABLED"
